...
public class AuthorService
{
    ...
    {
        return jdbcTemplate.queryForObject("SELECT * FROM author WHERE id = ?", (rs, rowNum) ->
                new AuthorEntity().setId(rs.getInt("id")).setName(rs.getString("name")), id
        );
    }

    public Map<Character, List<String>> getAuthorMapByFirstLetterOfSurname()
    {
        Character[] alphabet = new Character[] {'А', 'Б', 'В', 'Г', 'Д'
                ,'Е' ,'Ё' ,'Ж' ,'З' ,'И'
                ,'Й' ,'К' ,'Л' ,'М' ,'Н'
                ,'О' ,'П' ,'Р' ,'С' ,'Т'
                ,'У' ,'Ф' ,'Х' ,'Ц' ,'Ч'
